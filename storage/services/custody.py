...
class CustodyClient:
    ...
    async def prepare_content_encryption(self, content_id):
        async with aiohttp.ClientSession(headers=self.headers) as session:
            async with session.post(f"{self.url}/content/{content_id}/methods/prepare_encryption", headers=self.headers,
                                    json={}) as resp:
                print(resp.status)

    async def start_content_encryption(self, content_id, job_id):
        async with aiohttp.ClientSession(headers=self.headers) as session:
            async with session.post(f"{self.url}/content/{content_id}/methods/process_encryption", headers=self.headers,
                                    json={

...
async def create_user(*, db: dict = Depends(deps.get_db), user_in: schemas.UserCreate):
...
async def read_user_by_id(*, db: dict = Depends(deps.get_db), user_id: int):
...
async def update_user(
    *, db: dict = Depends(deps.get_db), user_id: int, user_in: schemas.UserUpdate
):
...
async def delete_user(*, db: dict = Depends(deps.get_db), user_id: int):
